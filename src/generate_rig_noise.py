...
noise_data_dir = "C:/Users/ramr/Documents/Github/personal_repos/allen_institute_projects/rig-noise.csv"
...
sweep_cols= ["cell_name", "inbath_sigma_long", "inbath_sigma_short", "cellatt_sigma_long", "cellatt_sigma_short", "breakin_sigma_long", "breakin_sigma_short"]
noise_cols= ["jem-date_patch", "cell_name", "inbath_sigma_long", "inbath_sigma_short", "cellatt_sigma_long", "cellatt_sigma_short", "breakin_sigma_long", "breakin_sigma_short"]
...
for cell_name in cell_list:
    if cell_name not in list(noise_df["cell_name"]):
        ...
        try: 
            (inbath_sigma_long, inbath_sigma_short) = calculate_std_vs(vs_inbath_sweep_nums)
            (cellatt_sigma_long, cellatt_sigma_short) = calculate_std_vs(vs_cellatt_sweep_nums)
            (breakin_sigma_long, breakin_sigma_short) = calculate_std_vs(vs_breakin_sweep_nums)
        
            sweep_df = pd.DataFrame(columns=sweep_cols)
            row_list = [cell_name, inbath_sigma_long, inbath_sigma_short, cellatt_sigma_long, cellatt_sigma_short, breakin_sigma_long, breakin_sigma_short]
            ...
            new_data_df.to_csv("rig-noise.csv", mode="a", index=False, header=False)
            print()
